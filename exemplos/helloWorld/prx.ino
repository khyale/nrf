...
void setup(){
  Serial.begin(9600);
  Serial.print("<< Teste de dispositivo no modo PRX >>\n\n");
}
...
void loop(){
  ...
  */ 
  ...
  rfmodule.set_rx_address(NRF_PIPE0,(uint8_t*)ptx_addr,5);
  rfmodule.set_rx_address(NRF_PIPE1,(uint8_t*)prx_addr,5);
  rfmodule.set_tx_address((uint8_t*)ptx_addr,5);
  rfmodule.set_retr_param(3,1); //retransmissao de 3 pacotes, timeout de 500us
  
  rfmodule.set_mode(NRF_RX_MODE);  //entra no modo RX
  /* escreve tudo o que chega do PTX na porta serial */
  while(true){
    if(rfmodule.available()){
      uint8_t buff[32],length;
      rfmodule.read_received_payload(buff,&length);
